...
axspre.set_title("Pre Angle Distribution")
axspre.grid()
...
# print histogram again to show more of even distribution of steering angles
hist, bins = np.histogram(angles, num_bins)
#removed for aws
figpost,axspost=plt.subplots()
axspost.bar(center, hist, align='center', width=width)
axspost.set_title("post angle distribution")
axspost.grid()
#plt.show()
...
print("steering angle is",angles[idx])
img = cv2.imread(imgt)
img=cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
# Draw a diagonal blue line with thickness of 5 px
pt2=(160,160)
pt1=(np.uint8(160*angleidx),110)

img_angled=np.copy(img)
img_angled=cv2.line(img_angled,pt1,pt2,(0,255,0),5)
...
def generate_training_data(image_paths, angles, batch_size=128, valid=False,top=50,bottom=140):
    ...
    while True:       
        for idx in range(len(angles)):
            #for dbg
            #print("img path:",image_paths[idx])
            #load image, BGR scheme
            img = cv2.imread(image_paths[idx])
            img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
            #for dbg
            #print("img path:",image_paths[idx],"imgshape:",img.shape)
            #get angle for the given image
            angle = angles[idx]            
            img=crop_image(img)
            #yuv/resize not ussed as drive.py kept crashing
            #img=yuv_image(img)
            #intensity change
            #plan was to add a random offset to #steering 
            #angle to emulate measurement noise
            #intensity augment is equivalent to adding measureent
            #noise in steering angle
            #this also helps model to generalize
            #rather than pick color as a feature
            img=intensity_image(img)
            #img=random_bright(img)
            #histogram equalization
            #img=equalize_image(img)
            #1/4 chance that image won't be transformed
            if not valid:
                flip_coin = random.randint(0,3)
                if flip_coin==1:
                    #add additonal image brightness
                    img=random_bright(img)
                elif flip_coin==2:
                    #translate pixels horizontally
            ...
            if len(Xt) == batch_size:
                ...
                Xt, yt = ([],[])
                image_paths, angles = shuffle(image_paths, angles)
            # flip images only for abs(angle) > -/+0.25
            #don't want to add images with low angle value
            #idea is to reduce low angle samples and augment
            #low occurence sample which in this case are angles
            # abs() > -/+ .25 so effectively their number doubles in training
...
axsend.set_title("End Angle Distribution")
axsend.grid()
#plt.show()
...
imga = cv2.imread(image_paths[rnd])
imga=cv2.cvtColor(imga,cv2.COLOR_BGR2RGB)
#print("img path:",image_paths[idx],"imgshape:",img.shape)
angle = angles[idx]

imgc=crop_image(imga)
#img=yuv_image(img)    

       
img_to_visualize=imgc
#removed for aws
